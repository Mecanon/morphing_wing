...
def plotter(i, database, s, l, x, y, J_x):
    ...
    image.append(plt.text(0, -0.17, 'T = %.0f K' % T))
    image.append(plt.text(0, -0.2, r'$\theta$ = %.1f$^{\circ}$' % math.degrees(theta)))
...
l = actuator(linear, J, zero_stress_length = 0.2533, material = 'linear')
