function [ sigma, MVF, T, eps_t, E, MVF_r, eps_t_r] = Full_Model_TC( t, eps, I, P, elastic_check, T_ambient)
...
for i = 2:size(t,1)
    ...
    rho_E = MVF(i,1)*P.rho_E_M + (1-MVF(i,1))*P.rho_E_A;
    r = (rho_E/P.rho)*(4*I(i,1)/(pi*P.d^2))^2;
    ...
    if strcmpi(elastic_check,'No') || strcmpi(elastic_check,'N')
        ...
        [ sigma(i,1), T(i,1), eps_t(i,1), MVF(i,1), H_cur(i,1), Phi_fwd(i,1), Phi_rev(i,1), chck ] = ...
            Elastic_Transformation_check_TC( P, TP, eps(i,1), eps(i-1,1), t(i,1), t(i-1,1), T(i-1,1), T(1,1), sigma(i-1,1), Phi_fwd(i-1,1), Phi_rev(i-1,1), E(i,1), MVF(i,1), eps_t(i,1), eps_t_r(i,1), MVF_r(i,1), r );
    
    elseif strcmpi(elastic_check,'Yes') || strcmpi(elastic_check,'Y')
        % Transformation surface rate-informed selected
        [ sigma(i,1), T(i,1), eps_t(i,1), MVF(i,1), H_cur(i,1), Phi_fwd(i,1), Phi_rev(i,1), chck ] = ...
            Elastic_Transformation_check_RI_TC(P, TP, eps(i,1), eps(i-1,1), t(i,1), t(i-1,1), T(i-1,1), T(1,1), sigma(i-1,1), Phi_fwd(i-1,1), Phi_rev(i-1,1), E(i,1), MVF(i,1), eps_t(i,1), eps_t_r(i,1), MVF_r(i,1), r );
