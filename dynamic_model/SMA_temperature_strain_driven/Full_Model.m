function [ sigma, MVF, eps_t, E, MVF_r, eps_t_r ] = Full_Model( T, eps, P, elastic_check, integration_scheme )
...
for i = 2:size(T,1)
    ...
    if strcmpi(elastic_check,'No') || strcmpi(elastic_check,'N')
        ...
        [ sigma(i,1), eps_t(i,1), MVF(i,1), H_cur(i,1), Phi_fwd(i,1), Phi_rev(i,1), chck ] = ...
            Elastic_Transformation_check( P, TP, eps(i,1), eps(i-1,1), T(i,1), T(i-1,1), T(1,1), sigma(i-1,1), Phi_fwd(i-1,1), Phi_rev(i-1,1), E(i,1), MVF(i,1), eps_t(i,1), eps_t_r(i,1), MVF_r(i,1) );
    
    elseif strcmpi(elastic_check,'Yes') || strcmpi(elastic_check,'Y')
        % Transformation surface rate-informed selected
        [ sigma(i,1), eps_t(i,1), MVF(i,1), H_cur(i,1), Phi_fwd(i,1), Phi_rev(i,1), chck ] = ...
            Elastic_Transformation_check_RI(P, TP, eps(i,1), eps(i-1,1), T(i,1), T(i-1,1), T(1,1), sigma(i-1,1), Phi_fwd(i-1,1), Phi_rev(i-1,1), E(i,1), MVF(i,1), eps_t(i,1), eps_t_r(i,1), MVF_r(i,1) );
